...
Процедура Печать(Команда)
	...
	Если Команда.Имя = "ПечатьБланков" ИЛИ Команда.Имя = "ПечатьБланковБыстрая" Тогда
		МакетПечати = "ПечатьБланковФ7";
	ИначеЕсли Команда.Имя = "ПечатьНаклеек" ИЛИ  Команда.Имя = "ПечатьНаклеекБыстрая" Тогда
		МакетПечати = "ПечатьНаклеек";
	ИначеЕсли Команда.Имя = "ПечатьЗаказов" ИЛИ  Команда.Имя = "ПечатьЗаказовБыстрая" Тогда
		МакетПечати = "ПечатьЗаказов";
	ИначеЕсли Команда.Имя = "ПечатьБланковПП" ИЛИ  Команда.Имя = "ПечатьБланковППБыстрая" Тогда
		МакетПечати = "ПечатьБланкПочтовыйПеревод";
	ИначеЕсли Команда.Имя = "ПечатьПервыхНаклеек" Тогда
		МакетПечати = "ПечатьПервыхНаклеек";
	КонецЕсли;
	...
	Если Команда.Имя = "ПечатьНаклеек" ИЛИ Команда.Имя = "ПечатьНаклеекБыстрая"
			ИЛИ Команда.Имя = "ПечатьПервыхНаклеек"  Тогда
	...
	Если Команда.Имя = "ПечатьБланков" Тогда
		ТДПечать.Показать("Печать: Бланк Ф7");
	ИначеЕсли  Команда.Имя = "ПечатьНаклеек" ИЛИ Команда.Имя = "ПечатьПервыхНаклеек" Тогда
		ТДПечать.Показать("Печать: Наклейки");
	ИначеЕсли  Команда.Имя = "ПечатьЗаказов" Тогда
		ТДПечать.Показать("Печать: Заказы");
	ИначеЕсли  Команда.Имя = "ПечатьБланковПП" Тогда
		ТДПечать.Показать("Печать: Бланк наложенного платежа");
	ИначеЕсли Команда.Имя = "ПечатьБланковБыстрая" ИЛИ Команда.Имя = "ПечатьНаклеекБыстрая" ИЛИ Команда.Имя = "ПечатьБланковППБыстрая" Тогда		
		ТДПечать.Напечатать();
	КонецЕсли;	
...
Процедура ПечатьЗаказовСервер(ТДПечать, МакетПечати, мсВыбранныеЗаказы)
	...
	Для Каждого Заказ ИЗ мсВыбранныеЗаказы Цикл 
		...
		//Проверить , если не умещается на странице, то вывести на следующй
		Если НЕ ТДПечать.ПроверитьВывод(ТДПечатнаяФормаДокумента)  Тогда 
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокЗаказов.Параметры.УстановитьЗначениеПараметра("СборПочты", Параметры.СборПочты);
	СписокЗаказов.Параметры.УстановитьЗначениеПараметра("СборКурьерки", Параметры.СборКурьерки);
	СписокЗаказов.Параметры.УстановитьЗначениеПараметра("DPD", Параметры.DPD);
	
	СписокЗаказов.Параметры.УстановитьЗначениеПараметра("мсЗаказыСНехваткойТовара",ПолучитьЗаказыСНехваткойОстатка());	
	
	Если Параметры.СборПочты Тогда
		Заголовок = "СБОР ПОЧТЫ";
	ИначеЕсли Параметры.СборКурьерки Тогда
		Заголовок = "СБОР КУРЬЕРКИ";
		Элементы.ФормаГруппаПечатьБланков.Видимость = Ложь;
	ИначеЕсли Параметры.DPD Тогда
		Заголовок = "DPD";
		Элементы.ФормаГруппаПечатьБланков.Видимость = Ложь;  
	КонецЕсли;
