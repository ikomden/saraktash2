...
Функция ОткрытьДляОтладки(Запрос, Изменять = Ложь) Экспорт
	Отладка = Истина;	
	Форма = ЭтотОбъект.ПолучитьФорму("Форма");
	Если Изменять тогда 
		ЗапросОтладки  = Запрос;
	Иначе
		ЗапросОтладки = Новый Запрос;
		ЗапросОтладки.Текст = Запрос.Текст;
		ЗапросОтладки.МенеджерВременныхТаблиц = Запрос.МенеджерВременныхТаблиц;
		Для Каждого КлючЗначение из Запрос.Параметры Цикл
			//( Интелис: Кравченко Денис 2012.07.26
			//БЫЛО:
			//ЗапросОтладки.Параметры.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
			//СТАЛО:
			ЗначениеПараметра = КлючЗначение.Значение;
			Если ТипЗнч(ЗначениеПараметра) = Тип("Массив") Тогда
				СписокЗначений = Новый СписокЗначений;
				СписокЗначений.ЗагрузитьЗначения(ЗначениеПараметра);				
				ЗначениеПараметра = СписокЗначений;
			КонецЕсли;
			ЗапросОтладки.Параметры.Вставить(КлючЗначение.Ключ, ЗначениеПараметра);
			//) Интелис: Кравченко Денис 2012.07.26
		КонецЦикла;
	КонецЕсли;
	//Форма.ЭлементыФормы.ТекстЗапроса.УстановитьТекст(Запрос.Текст);
	//++
	//Если Найти(Запрос.Текст,"ПОМЕСТИТЬ ") Тогда
	//	ФлагЗапросВременнаяТаблица = Истина;
	//	Форма.Открыть();
